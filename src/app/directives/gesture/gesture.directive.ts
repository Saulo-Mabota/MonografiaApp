...
export class GestureDirective implements OnInit {
  ...
  ngOnInit(): void {
    this.DIRECTIVE_GESTURES
      .filter((dGesture) => this.gestureOpts.find(({name}: {name: string}) => dGesture.name === name))
      .map((gesture) => {
        gesture.enabled = true;
        if (gesture.name === 'swipe') {
          const swipeGestureOpts = this.gestureOpts.find(({name}: {name: string}) => name == 'swipe');
  ...
  handleTaps(e: { timeStamp: any; }) {
    const tapTimestamp = Math.floor(e.timeStamp);
    const isDoubleTap = this.lastTap + (this.tapGesture?.interval ?? 0) > tapTimestamp;
    ...
    if (isDoubleTap && this.doubleTapGesture.enabled) {
      ...
      this.tapTimeout = setTimeout(() => this.emitTaps(), this.tapGesture?.interval ?? 0);
    }
  ...
  private handleMoving(moveType: any, $event: any) {
    ...
    const threshold:any = this.swipeGesture.threshold;
    ...
    if (shouldReport) {
      ...
      const dirArray:any = this.swipeGesture.direction;
